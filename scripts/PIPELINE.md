# PIPELINE.md — Veille & publication automatique Actualités, ACTU EYES

Reconstruit le 28/07/2026 après l'incident du 27/07/2026 (voir section dédiée en bas).
Ce fichier vit dans le dépôt Git (`Mika020202/actueyes-montreuil-site`), pas dans une session
...
2. **Encart source en fin d'article** : `<div class="article-source-note">Contenu
   rédigé par l'équipe ACTU EYES à partir de sources professionnelles vérifiées
...
1. `git clone https://github.com/Mika020202/actueyes-montreuil-site.git` (ou `git pull` si
...
8. Format attendu du message de fin d'exécution (sert de corps à l'email) :
   > Nouvel article publié sur actueyes-montreuil.fr : "[titre]" ([catégorie]).
   > URL : https://www.actueyes-montreuil.fr/actualites/[slug].html
